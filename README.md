...
- **3가지 비교 방식** (웹 앱에서 선택)
  ...
  3. **값 존재 비교 (있음/없음)** — 두 파일을 나란히 보여주고, 어떤 값이
     **양쪽 파일에 다 있으면 초록**, **한쪽 파일에만 있으면 빨강**으로 표시합니다
     (위치 무관·대칭). 예: A의 값이 B에도 있으면 초록, B에 없으면 빨강.
...
```shell
# 개발자 diff 뷰 (좌우 나란히, ~변경/-삭제/+추가)
$ python excel_diff.py 기존.xlsx 새.xlsx -o 비교결과.xlsx

# 값 존재 비교 · 양쪽에 있으면 초록 / 한쪽만 빨강
$ python excel_diff.py 기존.xlsx 새.xlsx -m presence -o 비교결과.xlsx
```

결과 워크북 구성: `[요약]` 시트(전체/시트별 집계 + 판정) + 원본 시트별 좌우 뷰.
`diff` 모드는 `~` 변경·노랑 / `-` 삭제·빨강 / `+` 추가·초록,
`presence` 모드는 초록(양쪽 파일에 다 있음) / 빨강(한쪽에만 있음)으로 칠합니다.
